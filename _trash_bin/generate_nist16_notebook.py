 ...
 "cells": [
  {
   ...
   "metadata": {},
   "source": [
    "# LMS-FNet: NIST16 SOTA Evaluation\n",
    "Domain Adaptation with Data Augmentation."
   ]
  },
  {
   "cell_type": "code",
  ...
  },
  {
   ...
   "metadata": {},
   "source": [
    "## 2. Dynamic Search for NIST16 Dataset"
   ]
  },
  {
   "cell_type": "code",
   ...
   "source": [
    "IMG_SIZE = (224, 224)\n",
    "BATCH_SIZE = 16\n",
    "CACHE_DIR = Path('/kaggle/working/nist16_cache')\n",
    "\n",
    "print(\"Scanning /kaggle/input/ for NIST16 Authentic and Tampered folders...\")\n",
    "input_path = Path('/kaggle/input')\n",
    "valid_exts = {'.jpg', '.jpeg', '.png', '.tif', '.tiff', '.bmp'}\n",
    "\n",
    "au_paths = [str(p) for p in input_path.rglob('*') if any(word in str(p).lower() for word in ['auth', 'pristine', 'world']) and p.is_file() and p.suffix.lower() in valid_exts]\n",
    "tp_paths = [str(p) for p in input_path.rglob('*') if any(word in str(p).lower() for word in ['tamp', 'probe', 'fake', 'manipulate']) and p.is_file() and p.suffix.lower() in valid_exts]\n",
    "\n",
    "print(f\"Found: {len(au_paths)} Authentic images\")\n",
    "print(f\"Found: {len(tp_paths)} Tampered images\")\n",
    "\n",
    "if len(au_paths) == 0 or len(tp_paths) == 0:\n",
    "    print(\"WARNING: NIST16 is primarily a localization dataset (tampered images only). If you don't have authentic images attached, this binary classification training will fail! Please ensure your Kaggle upload contains both classes.\")\n",
    "    raise ValueError(\"Balanced NIST16 dataset not found! Please attach a dataset containing both 'authentic' and 'tampered' folders.\")\n",
    "\n",
  ...
  },
  {
   ...
   "source": [
    ...
    "\n",
    "callbacks = [\n",
    "    tf.keras.callbacks.ModelCheckpoint('nist16_finetuned.keras', save_best_only=True, monitor='val_auc', mode='max'),\n",
    "    tf.keras.callbacks.EarlyStopping(monitor='val_auc', patience=5, restore_best_weights=True)\n",
    "]\n",
    "\n",
    "print(\"Starting Domain Adaptation on NIST16... (20 Epochs)\")\n",
    "history = model.fit(\n",
    "    train_ds,\n",
  ...
  },
  {
   ...
   "outputs": [],
   "source": [
    "print(\"Running Final Inference on NIST16 test set...\")\n",
    ...
    "\n",
    "print(\"\\n\" + \"=\"*50)\n",
    "print(\"NIST16 DOMAIN ADAPTATION RESULTS\")\n",
    "print(\"=\"*50)\n",
...
with open("LMS_FNet_NIST16_Domain_Adaptation.ipynb", "w", encoding='utf-8') as f:
